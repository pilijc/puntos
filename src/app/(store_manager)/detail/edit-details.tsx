...
import {
  useDetailStore,
  useDetailViewStore,
} from "@/store/store-manager/detail-store";
import {
  store_types_options,
  aspect_ratios,
  type PickImageType,
...
import {
  ...
  updateStoreDetail,
  uploadDetailImage,
} from "@/services/store-manager/detail-service";
import {
  useColorScheme,
  ActivityIndicator,
  Modal as RNModal,
...
function WebTimePicker({
  ...
  const currentM = String(Math.min(55, Math.round(rawM / 5) * 5)).padStart(
    2,
    "0",
  );
  ...
  return (
    <View className="flex-row items-center gap-x-2">
      ...
      <View style={{ flex: 1, zIndex: 20 }}>
        ...
        >
          <Text className="font-poppins text-slate-900 dark:text-darkTextPrimary">
            {currentH}
          </Text>
          <MaterialIcons
            name={showH ? "expand-less" : "expand-more"}
            size={16}
            color="#94A3B8"
          />
        </TouchableOpacity>
            ...
            {HOURS_24.map((h) => (
              <TouchableOpacity
                ...
                style={{
                  ...
                  paddingVertical: 8,
                  backgroundColor:
                    h === currentH ? "rgba(255,102,0,0.1)" : "transparent",
                }}
              >
                <Text
                  style={{
                    fontFamily: h === currentH ? "Poppins-SemiBold" : "Poppins",
                    fontSize: 13,
                    color:
                      h === currentH
                        ? "#FF6600"
      ...
      <View style={{ flex: 1, zIndex: 20 }}>
        ...
        >
          ...
          <MaterialIcons
            name={showM ? "expand-less" : "expand-more"}
            size={16}
            color="#94A3B8"
          />
        </TouchableOpacity>
            ...
            {MINUTES_5.map((m) => (
              <TouchableOpacity
                ...
                style={{
                  ...
                  paddingVertical: 8,
                  backgroundColor:
                    m === currentM ? "rgba(255,102,0,0.1)" : "transparent",
                }}
              >
                <Text
                  style={{
                    fontFamily: m === currentM ? "Poppins-SemiBold" : "Poppins",
                    fontSize: 13,
                    color:
                      m === currentM
                        ? "#FF6600"
...
export default function EditDetails() {
  ...
  const pickImage = async (kind: PickImageType, index?: number) => {
    ...
    if (!asset.base64) {
      showError(translate("storeManager.detailEdit.couldNotReadImage"));
      return;
    }
  ...
  const radiusCircle = React.useMemo(() => {
    ...
    return turf.circle([parsedLng, parsedLat], ((radius ?? 50) || 50) / 1000, {
      steps: 64,
      units: "kilometers",
    });
  ...
  const handleSave = async () => {
    if (!logo) {
      showError(translate("storeManager.detailEdit.logoRequired"));
      return;
    }
    if (!name.trim()) {
      showError(translate("storeManager.detailEdit.nameRequired"));
      return;
    }
    if (!type) {
      showError(translate("storeManager.detailEdit.typeRequired"));
      return;
    }

    const validPics = pictures.filter(Boolean) as string[];
    if (validPics.length === 0) {
      showError(translate("storeManager.detailEdit.picturesRequired"));
      return;
    }
    ...
    try {
      ...
      const uploadedPics = await Promise.all(
        validPics.map((p) => uploadDataUri(p, "picture")),
      );
      const uploadedDoc = businessDoc
        ? await uploadDataUri(businessDoc, "business_document")
        : null;
        ...
        >
          <View className="m-4 rounded-xl px-3 pt-2 pb-3 gap-y-3">
                ...
                {logo ? (
                  ...
                  <View className="flex-1 items-center justify-center gap-y-1">
                    <MaterialIcons
                      ...
                      color="#94A3B8"
                    />
                    <Text className="text-[9px] font-poppins text-slate-400">
                      {translate("storeManager.detailEdit.logo")}
                    </Text>
                  </View>
                )}
              </TouchableOpacity>
            ...
            <View
              ...
              style={isWeb ? { maxWidth: 300, width: "100%" } : undefined}
            >
              {[0, 1, 2].map((i) => {
                ...
                return (
                  <View
                    ...
                    style={isWeb ? { maxWidth: 90, minWidth: 0 } : undefined}
                  >
                    {uri ? (
                      ...
                      <TouchableOpacity
                        ...
                        activeOpacity={0.8}
                        style={
                          isWeb ? { maxWidth: 90, maxHeight: 90 } : undefined
                        }
                      >
                        <MaterialIcons
                          name="add-a-photo"
                          ...
                          color="#94A3B8"
                        />
                        <Text className="text-[9px] font-poppins text-slate-400 mt-0.5">
                          {translate("label.add")}
                        </Text>
                      </TouchableOpacity>
                    )}
          ...
          <View className="mx-4 rounded-xl px-3 pt-2 pb-3 gap-y-4">
            <Text className="text-base font-poppins-semibold text-textPrimary dark:text-darkTextPrimary">
              {translate(
                "storeManager.detailEdit.storeInformation",
                "Store Information",
              )}
            </Text>
            ...
            <TextField
              ...
              placeholder={translate(
                "storeManager.detailEdit.phonePlaceholder",
              )}
          ...
          <View
            className="mx-4 rounded-xl px-3 pt-2 pb-3 gap-y-4"
            style={{ zIndex: 10 }}
          >
            <Text className="text-base font-poppins-semibold text-textPrimary dark:text-darkTextPrimary">
              {translate(
                "storeManager.detailEdit.operatingHours",
                "Operating Hours",
              )}
            </Text>
          ...
          <View className="mx-4 rounded-xl overflow-hidden gap-y-4">
                  ...
                  >
                    ...
                    {radiusCircle && (
                      <Mapbox.ShapeSource id="radius" shape={radiusCircle}>
                        <Mapbox.FillLayer
                          id="radiusFill"
                          style={{ fillColor: "#FF6600", fillOpacity: 0.14 }}
                        />
                      </Mapbox.ShapeSource>
                    )}
                  </MapView>
            ...
            <View className="px-3 gap-y-4">
              <TextField
                ...
                placeholder={translate(
                  "storeManager.detailEdit.addressPlaceholder",
                )}
                value={address}
          ...
          <View className="m-4 rounded-xl px-3 pt-2 pb-3 gap-y-3">
            ...
            <TouchableOpacity
              onPress={() => {
                if (!businessDoc) pickImage("business_document");
              }}
          ...
          <View className="m-4 gap-y-3">
            <Button
              label={
                isUploading
                  ? translate("storeManager.detailEdit.uploadingImages")
          ...
          <RNModal visible transparent animationType="slide">
            ...
            >
              <TouchableOpacity
                ...
                className="bg-white dark:bg-darkBackgroundCard rounded-t-2xl pb-8 pt-2"
              >
                <DateTimePicker
                  value={timeStringToDate(storeOpen || "09:00", 9, 0)}
                  mode="time"
