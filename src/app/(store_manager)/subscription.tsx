...
import { ActivityIndicator, Linking, Platform, useColorScheme } from "react-native";
...
export default function SubscriptionScreen() {
  ...
  const { startingCheckout, cancellingSubscription, modal, clearModal } =
  ...
  const loading =
    ...
    subQuery.isPending ||
    (Boolean(ownerId) && invoicesQuery.isPending);
  ...
  const activePlanName = useMemo(() => {
    ...
    if (currentSubscriptionId == null)
      return String(
        basicPlan?.name ??
          translate(
            "storeManager.subscription.billing.fallbackPlanName.basic",
            "Basic plan",
          ),
      );
    if (proId != null && currentSubscriptionId === proId)
      return String(
        proPlan?.name ??
          translate(
            "storeManager.subscription.billing.fallbackPlanName.pro",
            "Pro plan",
          ),
      );
    ...
    return String(
      proPlan?.name ??
        basicPlan?.name ??
        translate("storeManager.subscription.title", "Plan"),
    );
  }, [
    basicId,
    basicPlan?.name,
  ...
  const isBasicPlanSelected =
    ...
    (basicId != null && currentSubscriptionId === basicId);
  const isBasicFree =
    !isPaidPro && isBasicPlanSelected && upcomingBillingAmount === 0;
  ...
  const accessEndMessage = useMemo(() => {
    ...
    if (!end) return null;
    return translate(
      "storeManager.subscription.billing.messages.accessUntilNoBill",
      {
  ...
  const invoiceColumns = useMemo((): Array<
    TableColumn<ManagerSubscriptionPaymentRow>
  > => {
    return [
      {
        key: "payment_reference",
        header: translate(
          "storeManager.subscription.billing.invoices.columns.paymentReference",
        ),
        flex: 3,
        align: "left",
        render: (r) => (
      ...
      },
      {
        key: "paid_date",
        header: translate(
          "storeManager.subscription.billing.invoices.columns.paidDate",
        ),
        flex: 2,
        align: "center",
        render: (r) => (
      ...
      },
      {
        key: "status",
        header: translate(
          "storeManager.subscription.billing.invoices.columns.status",
        ),
        flex: 1,
        align: "center",
        render: (r) => {
          ...
          return (
            <View className={`px-2 py-0.5 rounded-full ${style.container}`}>
              <Text
                className={`text-xs font-poppins-bold uppercase ${style.text}`}
              >
                {status || "—"}
              </Text>
            </View>
      ...
      },
      {
        key: "amount",
        header: translate(
          "storeManager.subscription.billing.invoices.columns.amount",
        ),
        flex: 1,
        align: "right",
        render: (r) => (
    ...
    async (amount: number, name: string) => {
      ...
      if (!ownerId) {
        s.showMessage(
          ...
          translate(
            "storeManager.subscription.billing.errors.mustBeSignedInToSubscribe",
          ),
        );
        return;
      }
      ...
      try {
        ...
        const checkoutUrl = await useSubscriptionCheckout(
          ownerId,
          selectedSlug,
          amount,
          name,
        ...
        if (checkoutUrl) {
          ...
          s.showMessage(
            translate("label.somethingWentWrong"),
            translate(
              "storeManager.subscription.billing.errors.checkoutStartFailed",
            ),
          );
        }
      } catch (e) {
        s.showMessage(
          ...
          e instanceof Error
            ? e.message
            : translate(
                "storeManager.subscription.billing.errors.pleaseTryAgain",
              ),
        );
      } finally {
  ...
  const performCancel = useCallback(async () => {
    ...
    try {
      ...
      if (result.ok === false) {
        s.showMessage(
          translate("storeManager.subscription.billing.cancel.failedTitle"),
          result.error,
        );
        return;
      }
  ...
  const openCancelConfirm = useCallback(() => {
    ...
    s.setModal({
      ...
      message: translate(
        "storeManager.subscription.billing.cancel.confirmMessage",
      ),
      buttons: [
        {
        ...
        {
          label: translate(
            "storeManager.subscription.billing.cancel.confirmCta",
          ),
          variant: "danger",
          onPress: () => {
            s.clearModal();
              ...
              <View className="flex-row gap-6 mt-5">
                ...
                {shouldShowUpcomingBill ? (
                  <View className="flex-1">
                    <Text className="text-xs font-poppins-semibold text-white/80">
                      {translate(
                        "storeManager.subscription.billing.upcomingAmount",
                      )}
                    </Text>
                    <Text className="mt-1 text-sm font-poppins-semibold text-white">
                    ...
                    <Text className="mt-1 text-[10px] font-poppins text-white/80 leading-4">
                      {translate(
                        "storeManager.subscription.billing.upcomingPriceNote",
                      )}
                    </Text>
                  </View>
                ) : null}
              ...
              <View className="flex-row flex-wrap gap-3 mt-6 justify-start">
                {!isPaidPro && proPlan ? (
                  <Button
                    variant="accent"
                    label={translate(
                      "storeManager.subscription.billing.upgradeToPro",
                    )}
                    roundedFull
                    icon="Sparkles"
                    elevation={true}
                ...
                {isPaidPro && !cancelScheduled ? (
                  <Button
                    ...
                    fitContent={true}
                    label={translate(
                      "storeManager.subscription.billing.cancelSubscription",
                    )}
                    roundedFull
          ...
          <View className="rounded-xl border border-slate-100 dark:border-darkBorder p-4">
            <View className="flex-row items-center gap-2 mb-2">
              <Text className="text-sm font-poppins-semibold text-slate-800 dark:text-darkTextPrimary">
                {translate(
                  "storeManager.subscription.billing.recentBilling.title",
                )}
              </Text>
            </View>
            <Text className="text-xs font-poppins text-slate-500 dark:text-darkTextMuted leading-4 mb-4 px-0.5">
              {translate(
                "storeManager.subscription.billing.recentBilling.subtitle",
              )}
            </Text>
            ...
            {loadingInvoices ? (
              ...
              <>
                <Table
                  ...
                  emptyText={translate(
                    "storeManager.subscription.billing.invoices.empty",
                  )}
                />
                <View className="mt-3 items-center">
                  <Text className="text-xs font-poppins text-slate-400 dark:text-darkTextSecondary">
                    {translate(
                      "storeManager.subscription.billing.invoices.end",
                    )}
                  </Text>
                </View>
              </>
