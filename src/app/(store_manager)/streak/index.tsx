...
export default function ViewStreak() {
  ...
  const {
    ...
    loading: permLoading,
    expiresAtIso,
  } = useStorePremiumCampaignEdit(storeId);
  ...
  const upcomingStreaks = streaks.filter(
    (s) => s.status === "draft" || s.status === "upcoming",
  );
  ...
  const hasMoreUpcoming =
    visibleUpcomingStreaks.length < upcomingStreaks.length;
  ...
  const streakErrorTitle = (messageKey: string | null) => {
    ...
    if (isStreakNoticeMessageKey(messageKey))
      return translate("label.almostThere");
  ...
  const showStreakError = (e: unknown) => {
    ...
    setModal({
      ...
      message,
      buttons: [
        { label: translate("label.ok"), onPress: () => setModal(null) },
      ],
    });
  };
  ...
  const handleActivate = (streak: Streak) => {
    setModal({
      ...
      buttons: [
        ...
        {
          label: translate("storeManager.streak.activate"),
          variant: "primary",
          onPress: () => {
            setModal(null);
            doActivate(streak.id!);
          },
        },
      ],
    });
  ...
  const handleEnd = (streak: Streak) => {
    setModal({
      ...
      buttons: [
        ...
        {
          label: translate("storeManager.streak.endProgram"),
          variant: "danger",
          onPress: () => {
            setModal(null);
            doEnd(streak.id!);
  ...
  const handleDelete = (streak: Streak) => {
    setModal({
      ...
      buttons: [
        ...
        {
          label: translate("storeManager.streak.deleteProgram"),
          variant: "danger",
          onPress: () => {
            setModal(null);
            doDelete(streak.id!);
  ...
  const handleLoadMore = () => {
    if (activeTab === "upcoming" && hasMoreUpcoming) {
      setUpcomingVisible(
        Math.min(upcomingVisible + PAGE_SIZE, upcomingStreaks.length),
      );
      return;
    }
    ...
    >
      ...
      {campaignsLocked ? (
        <View className={isWeb ? "px-4 pt-3 items-center" : "px-4 pt-3"}>
          <View
            className={`w-full rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 px-3 py-2.5 ${isWeb ? "max-w-4xl" : ""}`}
          >
            <Text className="text-xs font-poppins text-amber-900 dark:text-amber-200 leading-5">
              {expiresAtIso
                ? translate("storeManager.premiumCampaigns.bannerWithExpiry", {
                    date: formatDate(expiresAtIso),
                  })
          ...
          <FlatList
            ...
            renderItem={({ item: streak }) => (
              <StreakCard
                ...
                onPublish={
                  streak.status === "draft"
                    ? () => handlePublish(streak)
                    : undefined
                }
                onActivate={
                  streak.status === "upcoming"
                    ? () => handleActivate(streak)
                    : undefined
                }
                onEnd={
                ...
                onDelete={
                  streak.status === "draft" || streak.status === "upcoming"
                    ? () => handleDelete(streak)
                    : undefined
                }
                isPublishing={
                  acting?.id === streak.id && acting?.action === "publish"
                }
                isActivating={
                  acting?.id === streak.id && acting?.action === "activate"
                }
                isEnding={acting?.id === streak.id && acting?.action === "end"}
                isDeleting={
                  acting?.id === streak.id && acting?.action === "delete"
                }
              />
            )}
          />
