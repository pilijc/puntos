...
export default function CreateStore() {
  ...
  const getCreateStoreErrorMessage = (error: unknown) => {
    ...
    if (
      ...
      return translate("storeManager.createStore.storeHoursRequired");
    }

    return translate("storeManager.createStore.createFailedMessage");
  };
  ...
  const pickImage = async (type: PickImageType, pictureIndex?: number) => {
    ...
    if (!asset.base64) {
      showError(translate("storeManager.createStore.couldNotReadImage"));
      return;
    }
  ...
  const handleGetCurrent = async () => {
    ...
    if (!loc) {
      showError(translate("storeManager.createStore.couldNotGetLocation"));
      return;
    }
  ...
  const submitCreateStore = async () => {
    try {
      ...
      setModal({
        title: translate("storeManager.createStore.storeCreatedTitle"),
        message: translate("storeManager.createStore.storeCreatedMessage"),
        buttons: [
          {
            label: translate("storeManager.createStore.viewStore"),
            variant: "primary",
            onPress: () => {
              setModal(null);
  ...
  return (
    <SafeAreaView edges={["top", "left", "right"]} className="flex-1 bg-backgroundMuted dark:bg-[#111921]">
      ...
      />
      <AppHeader
        title={translate("storeManager.createStore.title")}
        onBackPress={() => {
            ...
            <View className="flex-row gap-3">
              {activeStep !== "store" && (
                <View className="flex-1">
                  <Button
                    label={translate("storeManager.createStore.back")}
                    onPress={goBack}
                    variant="secondary"
                    loading={false}
                    fullWidth
              ...
              <View className="flex-1">
                <Button
                  label={
                    activeStep === "location"
                      ? translate("storeManager.createStore.createStore")
                      : translate("storeManager.createStore.continue")
                  }
                  onPress={goNext}
                  variant="primary"
